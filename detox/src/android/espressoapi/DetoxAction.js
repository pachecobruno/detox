...
class DetoxAction {
  ...
  static scrollInDirection(direction, amountInDP) {
    if (typeof direction !== 'string')
      throw new Error('direction should be a string, but got ' + (direction + (' (' + (typeof direction + ')'))));
    if (typeof amountInDP !== 'number')
      throw new Error('amountInDP should be a number, but got ' + (amountInDP + (' (' + (typeof amountInDP + ')'))));
    return {
      target: {
        type: 'Class',
        value: 'com.wix.detox.espresso.DetoxAction'

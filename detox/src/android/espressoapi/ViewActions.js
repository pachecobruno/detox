...
class ViewActions {
	...
	static typeTextIntoFocusedView(stringToBeTyped) {
		if (typeof stringToBeTyped !== 'string')
			throw new Error('stringToBeTyped should be a string, but got ' + (stringToBeTyped + (' (' + (typeof stringToBeTyped + ')'))));
		return {
			target: {
				type: 'Class',
				value: 'android.support.test.espresso.action.ViewActions'
	...
	static replaceText(stringToBeSet) {
		if (typeof stringToBeSet !== 'string')
			throw new Error('stringToBeSet should be a string, but got ' + (stringToBeSet + (' (' + (typeof stringToBeSet + ')'))));
		return {
			target: {
				type: 'Class',
				value: 'android.support.test.espresso.action.ViewActions'
	...
	static openLinkWithText(linkText) {
		if (typeof linkText !== 'string')
			throw new Error('linkText should be a string, but got ' + (linkText + (' (' + (typeof linkText + ')'))));
		return {
			target: {
				type: 'Class',
				value: 'android.support.test.espresso.action.ViewActions'

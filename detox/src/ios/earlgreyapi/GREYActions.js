...
*/ static actionForLongPressWithDuration(
    ...
    if (typeof duration !== 'number')
      throw new Error('duration should be a number, but got ' + (duration + (' (' + (typeof duration + ')'))));
    return {
      target: {
        type: 'Class',
        value: 'GREYActions'
...
*/ static actionForScrollInDirectionAmount(
    ...
    if (!['left', 'right', 'up', 'down'].some((option) => option === direction))
      throw new Error('direction should be one of [left, right, up, down], but got ' + direction);
...
*/ static actionForScrollInDirectionAmountXOriginStartPercentageYOriginStartPercentage(
    ...
    if (typeof xOriginStartPercentage !== 'number')
      throw new Error(
        'xOriginStartPercentage should be a number, but got ' + (xOriginStartPercentage + (' (' + (typeof xOriginStartPercentage + ')')))
      );
    if (typeof yOriginStartPercentage !== 'number')
      throw new Error(
        'yOriginStartPercentage should be a number, but got ' + (yOriginStartPercentage + (' (' + (typeof yOriginStartPercentage + ')')))
      );
    return {
      target: {
        type: 'Class',
...
*/ static actionForScrollToContentEdge(edge) {
    if (!['left', 'right', 'top', 'bottom'].some((option) => option === edge))
      throw new Error('edge should be one of [left, right, top, bottom], but got ' + edge);
    return {
      target: {
        type: 'Class',
        value: 'GREYActions'
...
*/ static actionForMultiFingerSwipeSlowInDirectionNumberOfFingers(
    ...
    if (typeof numberOfFingers !== 'number')
      throw new Error('numberOfFingers should be a number, but got ' + (numberOfFingers + (' (' + (typeof numberOfFingers + ')'))));
    return {
      target: {
        type: 'Class',
        value: 'GREYActions'

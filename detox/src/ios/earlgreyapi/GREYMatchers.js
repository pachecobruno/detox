...
*/ static matcherForAccessibilityID(
    ...
    if (typeof accessibilityID !== 'string')
      throw new Error('accessibilityID should be a string, but got ' + (accessibilityID + (' (' + (typeof accessibilityID + ')'))));
    return {
      target: {
        type: 'Class',
        value: 'GREYMatchers'
...
*/ static matcherForScrollViewContentOffset(
    ...
    if (typeof offset.x !== 'number')
      throw new Error('offset.x should be a number, but got ' + (offset.x + (' (' + (typeof offset.x + ')'))));
    if (typeof offset.y !== 'number')
      throw new Error('offset.y should be a number, but got ' + (offset.y + (' (' + (typeof offset.y + ')'))));
    return {
      target: {
        type: 'Class',
        value: 'GREYMatchers'
